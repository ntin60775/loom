...
### 16.3 Multi-agent coordination (параллельные workers)

**Что это.** Сейчас agent mode строго последовательный: шаг 1 → worker → reviewer → шаг 2. Но некоторые шаги плана независимы (меняют разные файлы). Их можно выполнять параллельно.

**Почему не в v1.** Параллельные workers создают проблемы:
- Git conflicts — оба меняют соседние файлы
- Согласованность — worker A меняет интерфейс, worker B его использует но не видит изменений
- Review ordering — reviewer должен видеть полную картину, а не два изолированных diff
- Deadlocks — worker A ждёт B, который ждёт A

Проще сделать строго последовательно, отладить базовый цикл, а потом добавлять параллелизм с явными dependency constraints в plan.json.

**Как в v2.** Executor анализирует `depends_on` в plan.json и строит DAG. Независимые шаги → параллельные workers в tmux. Мерж через отдельные ветки для каждого worker → reviewer видит объединённый diff.

### 16.4 Как эти слои связаны

```
Memory Layer
    │ питает фактами и историей
    ▼
Scout Retrieval (не эмбеддинги)
    │ находит релевантные знания для контекста
    ▼
Multi-Agent Coordination
    │ использует полный контекст для параллельной работы
    ▼
Более умный Executor
```

Это дорожная карта «интеллектуализации» на v2/v3. v1 — механика, которая должна заработать в однопоточном режиме. Когда стабильна — добавляем мозги.
